...
export default function Home() {
    ...
    const addTodo = () => {
        ...
        if (todos) {
            ...
            if (todosJson.filter(value => { return value.title == todo.title }).length > 0) {
                ...
                setTodo({ title: "", desc: "" }) //todos gayab hone k liye
            }
        } else {
    ...
    return ( <
        ...
        label
        for = "title"
        class = "leading-7 text-sm text-gray-600" > Todo Title < /label> <
        ...
        label
        for = "desc"
        class = "leading-7 text-sm text-gray-600" > Todo Text < /label> <
        input onChange = { onChange }
        value = { todo.desc }
        type = "text"
        id = "desc"
        ...
        class = "text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none w-fit hover:bg-indigo-600 rounded text-lg" > Add Todo < /button> <
